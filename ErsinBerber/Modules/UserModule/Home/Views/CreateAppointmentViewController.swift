...
//        let size = (view.frame.size.width / 5) - 12
...
class CreateAppointmentViewController: UIViewController, UICollectionViewDataSource, UICollectionViewDelegate {
    ...
    override func viewDidLayoutSubviews() {
        ...
        timesCollectionView.frame = CGRect(x: width * 0.05, y: timeLabel.frame.maxY + 10, width: width * 0.9, height: height * 0.2)
        
        // ScrollView İçeriği Ayarla
        scrollView.contentSize = CGSize(width: width, height: timesCollectionView.frame.maxY + 20)
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: TimesCollectionViewCell.identifier, for: indexPath) as! TimesCollectionViewCell
        let time = workingHours[indexPath.row]
