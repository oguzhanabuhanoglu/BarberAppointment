...
class BarberLogInViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    @objc func didTapLogin() {
        ...
        DatabaseManager.shared.findBarber(with: fullPhoneNumber) { barber in
            if let barber = barber {
                
                AuthManager.shared.sendVerificationCode(phoneNumber: fullPhoneNumber) { success, error in
                    if success {
                        let destinationVC = BarberVerificationViewController()
                        destinationVC.barber = barber
                ...
                self.makeAlert(title: "Upps 🧐", message: "Something went wrong")
            }
        }
